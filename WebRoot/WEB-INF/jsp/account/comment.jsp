									...
									<c:if test="${!empty mcList}">
										<c:forEach items="${mcList}" var="m">
											<tr style="height: 60px;">
												...
												<td width="50%" align="center" class="td2">
													<form action="myTravels/toShow.do" method="post"
														id="myForm2<%=count%>">
														<input type="hidden" name="id" value="${m.id}" />
													</form>

													<form action="myComment1/toDelete.do" method="post"
														id="myForm1<%=count%>">
														<input type="hidden" name="id" value="${m.id}" />
													</form>

										
																<a href="javascript:viod(0)" onclick="sc('<%=count++%>')" align="center">删除</a>
