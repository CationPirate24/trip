      ...
      <div class="templatemo-content col-1 light-gray-bg">
        ...
        <div class="templatemo-content-container">
          <div class="templatemo-content-widget white-bg">
            <h2 class="margin-bottom-10">修改管理员信息</h2>
            <p>Here goes another form and form controls.</p>
            <form action="manager/update.do" class="templatemo-login-form" method="post" enctype="multipart/form-data">
              <input type="hidden" name="id" value="${mm.id}"/>
              <input type="hidden" name="username" value="${mm.username}"/>
              <input type="hidden" name="password" value="${mm.password}"/>
              
              <input type="hidden" name="name" value="${mm.name}"/>
              <input type="hidden" name="age" value="${mm.age}"/>
              <input type="hidden" name="sex" value="${mm.sex}"/>
              <input type="hidden" name="email" value="${mm.email}"/>
              <input type="hidden" name="phone" value="${mm.phone}"/>
              
              <div class="row form-group">
                <div class="col-lg-12 has-success form-group">                  
                    <label class="control-label" for="inputWithSuccess">姓名：${mm.name}</label>
                </div>
              </div>
              <div class="row form-group">
                <div class="col-lg-12 has-warning form-group">                  
                    <label class="control-label" for="inputWithWarning">年龄：${mm.age}</label>
                </div>
              </div>
              <div class="row form-group">
                <div class="col-lg-12 has-error form-group">                  
                    <label class="control-label" for="inputWithError">性别：${mmSex}</label>
                </div>
              </div>
              ...
              <div class="row form-group">
                <div class="col-lg-12 has-success form-group">                   
                    <div class="margin-right-15 templatemo-inline-block">
                      <label class="control-label" for="inputWithSuccess">邮箱：${mm.email}</label>
                    </div>
                </div>
              </div>
              ...
              <div class="row form-group">
                <div class="col-lg-12 has-warning form-group">                   
                    <label class="control-label" for="inputWithWarning">电话：${mm.phone}</label>
                </div>
              </div>
              ...
              <div class="row form-group">
                <div class="col-lg-12 form-group">  
                	<c:forEach items="${typeList}" var="t">                 
	                    <div class="margin-right-15 templatemo-inline-block">
	                      <input type="radio" name="type" id="r<%=count2%>" value="${t.myKey}">
	                      <label for="r<%=count2++%>" class="font-weight-400"><span></span>${t.myValue}</label>
	                    </div>
                    </c:forEach>
                </div>
              </div>
