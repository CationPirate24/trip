...
<html lang="en">
  <head>
    ...
    <script type="text/javascript">
		function add(){
			window.location.href = "managerUser/toAdd.do";
		}
		function lb(){
			window.location.href = "managerUser/toList.do";
		}
		function cx(){
    ...
    <div class="templatemo-flex-row">
      ...
      <div class="templatemo-content col-1 light-gray-bg">
        ...
        <div class="templatemo-content-container">
          <div id="myDiv">
          	<form action="managerUser/toList.do" method="post" id="myForm1">
	          	<table>
	          		<tr>
	          			<%--<td>
                ...
                <tbody>
                  <c:forEach items="${maList}" var="ma">
	                  <tr>
						...
						<td>${ma.phone}</td>
	                    <td>
							<form id="myFormFind<%=count%>" action="managerUser/toFind.do" method="post">
								<input type="hidden" name="id" value="${ma.id}"/>
							</form>
	                    	<a href="javascript:void(0)" onclick="find('<%=count%>')" class="templatemo-edit-btn">查看</a>
	                    
							<%--<form id="myFormUpdate<%=count%>" action="managerUser/toUpdate.do" method="post">
								<input type="hidden" name="id" value="${ma.id}"/>
							</form>
	                    	<a href="javascript:void(0)" onclick="update('<%=count%>')" class="templatemo-edit-btn">修改</a>
	                    
							--%><form id="myFormDelete<%=count%>" action="managerUser/delete.do" method="post">
								<input type="hidden" name="id" value="${ma.id}"/>
							</form>
          ...
          <div class="pagination-wrap">
            <ul class="pagination">
              ...
              <li><a href="managerUser/toList.do?currentPage=1">首页</a></li>
    		 ...
    		 %>
    		  ...
    		  %>
    		  <li><a href="managerUser/toList.do?currentPage=<%=currentPage-1%>">上一页</a></li>
    		  <%
    		  	}
    		   %>
    		...
    		<%
    		 ...
    		 <%
    		  ...
    		  %>
    		  	<li><a href="managerUser/toList.do?currentPage=<%=currentPage+1%>&con=${con}">下一页</a></li>
    		  <%
    		  	}
    		   %>
              <li><a href="managerUser/toList.do?currentPage=<%=pageCount%>">尾页</a></li>
